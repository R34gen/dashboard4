...
if page == "Executive Overview":
    ...
    with left:
        panel_open("Category Performance Portfolio", "Membaca kategori dari GMV, review, volume order, dan tingkat keterlambatan.")
        ...
        if not category.empty and needed.issubset(category.columns):
            ...
            st.info("Data category performance belum lengkap untuk visual portfolio.")
        panel_close()
    with right:
...
# -----------------------------------------------------------------------------
    ...
    with left:
        panel_open("Revenue Contribution by Segment", "Membaca kontribusi revenue per segmen pelanggan secara terurut.")
...
# -----------------------------------------------------------------------------
    ...
    context_box(
        ...
        "Fokus halaman ini bukan menjual model sebagai prediksi sempurna, tetapi membuktikan dampak keterlambatan terhadap review dan memakai model sebagai baseline screening untuk prioritas monitoring."
    )
    ...
    if not model_result.empty:
        ...
        roc = safe_float(model_result.get("roc_auc", pd.Series([0])).iloc[0])
        ap = safe_float(model_result.get("average_precision", pd.Series([0])).iloc[0])
        baseline = safe_float(model_result.get("late_rate_baseline", pd.Series([0])).iloc[0])

    def delivery_metric(status, col, default=0):
        if late_review.empty or "delivery_status" not in late_review.columns or col not in late_review.columns:
            return default
        hit = late_review.loc[
            late_review["delivery_status"].astype(str).str.lower().eq(status.lower()), col
        ]
        return safe_float(hit.iloc[0], default) if not hit.empty else default

    ontime_avg = delivery_metric("On Time", "avg_review")
    late_avg = delivery_metric("Late", "avg_review")
    review_drop = max(0, ontime_avg - late_avg)
    ontime_low = delivery_metric("On Time", "low_review_rate")
    late_low = delivery_metric("Late", "low_review_rate")
    low_lift = late_low / ontime_low if ontime_low else 0
    ontime_days = delivery_metric("On Time", "avg_delivery_days")
    late_days = delivery_metric("Late", "avg_delivery_days")
    days_extra = max(0, late_days - ontime_days)
    pr_lift = ap / baseline if baseline else 0

    c1, c2, c3, c4 = st.columns(4)
    c1.markdown(metric_card("Review Drop", f"-{fmt_float(review_drop)}", "Late orders vs on-time orders"), unsafe_allow_html=True)
    c2.markdown(metric_card("Low Review Lift", f"{fmt_float(low_lift)}×", "Late orders are more likely to get low review", "cyan"), unsafe_allow_html=True)
    c3.markdown(metric_card("Best Screening Model", str(selected_model), "Selected from model comparison", "dark"), unsafe_allow_html=True)
    c4.markdown(metric_card("PR Lift", f"{fmt_float(pr_lift)}×", f"Average precision vs baseline late rate {fmt_pct(baseline)}", "dark"), unsafe_allow_html=True)

    st.markdown("""
    <div class="context-box">
        <b>Reading rule</b><br>
        Bukti utama halaman ini adalah <span>service impact</span>: order terlambat memiliki review lebih rendah. Model hanya dipakai sebagai <span>screening layer</span> untuk membantu prioritas monitoring, bukan untuk keputusan otomatis.
    </div>
    """, unsafe_allow_html=True)

    left, right = st.columns([1.15, 0.85])
    with left:
        panel_open("Service Impact Curve", "Perbandingan review rata-rata antara order tepat waktu dan terlambat. Visual ini lebih kuat daripada risk bucket karena langsung menunjukkan dampak ke customer experience.")
        if not late_review.empty and {"delivery_status", "avg_review"}.issubset(late_review.columns):
            ...
            order = ["On Time", "Late"]
            lr["delivery_label"] = pd.Categorical(lr["delivery_label"], categories=order, ordered=True)
            lr = lr.sort_values("delivery_label")
            fig = go.Figure()
            fig.add_trace(go.Scatter(
                x=lr["delivery_label"],
                y=lr["avg_review"],
                mode="lines+markers+text",
                text=[fmt_float(v) for v in lr["avg_review"]],
                textposition="top center",
                name="Average Review",
                line=dict(width=5),
                marker=dict(size=18, line=dict(width=2, color="rgba(255,255,255,.55)")),
            ))
            fig.add_shape(
            ...
            fig.add_annotation(
                x="Late", y=late_avg,
                text=f"Review turun {fmt_float(review_drop)} poin",
                showarrow=True, arrowhead=2, ax=-70, ay=-45,
                font=dict(color="#FCA5A5", size=13),
                bgcolor="rgba(15,23,42,0.90)", bordercolor="rgba(248,113,113,0.45)", borderpad=6
            )
            fig.update_yaxes(range=[0, 5], title="Average Review Score")
            fig.update_xaxes(title="Delivery Status")
            st.plotly_chart(style_fig(fig, 390, legend=False), use_container_width=True, config=PLOT_CONFIG)
        panel_close()

    with right:
        panel_open("Experience Profile", "Profil ringkas keterlambatan: bukan tabel, tetapi kartu pembanding yang bisa dibaca cepat.")
        status_rows = [
            ("On Time", ontime_avg, ontime_low, ontime_days, delivery_metric("On Time", "orders")),
            ("Late", late_avg, late_low, late_days, delivery_metric("Late", "orders")),
        ]
        for i, (status, avg, low, days, orders) in enumerate(status_rows, start=1):
            marker = "✓" if status == "On Time" else "!"
            st.markdown(f"""
            <div class="rank-card">
                <div class="rank-left">
                    <div class="rank-no">{marker}</div>
                    <div>
                        <div class="rank-name">{status}</div>
                        <div class="rank-caption">Orders: {fmt_num(orders)} • Avg days: {fmt_float(days)} • Low review: {fmt_pct(low)}</div>
                    </div>
                </div>
                <div class="rank-value">{fmt_float(avg)}</div>
            </div>
            """, unsafe_allow_html=True)
        st.markdown(f"""
        <div class="insight-card">
            <div class="tag">Main evidence</div>
            <div class="card-title">Late delivery hurts review</div>
            <div class="card-body">Review turun dari <b>{fmt_float(ontime_avg)}</b> menjadi <b>{fmt_float(late_avg)}</b>. Ini lebih defensible daripada menjadikan model sebagai klaim utama.</div>
            <div class="card-body card-rec">Action: monitoring SLA dan seller/logistic performance.</div>
        </div>
        """, unsafe_allow_html=True)
        panel_close()
    ...
    with left:
        panel_open("Model Selection Map", "Peta pemilihan model: kanan-atas lebih baik. Average Precision penting karena late delivery adalah kelas minoritas.")
        if not model_cmp.empty and {"model", "average_precision", "roc_auc"}.issubset(model_cmp.columns):
            cmp = model_cmp.copy()
            size_col = "recall_late" if "recall_late" in cmp.columns else "f1_late" if "f1_late" in cmp.columns else None
            fig = px.scatter(
                cmp,
                x="roc_auc",
                y="average_precision",
                ...
                labels={
                    ...
                    "recall_late": "Recall Late",
                    "f1_late": "F1 Late"
                },
                size_max=42,
            )
            fig.add_vline(x=roc, line_dash="dot", line_color="rgba(255,255,255,.35)")
            fig.add_hline(y=ap, line_dash="dot", line_color="rgba(255,255,255,.35)")
            fig.update_traces(textposition="top center", marker=dict(line=dict(width=1, color="rgba(255,255,255,.45)")))
            st.plotly_chart(style_fig(fig, 390, legend=False), use_container_width=True, config=PLOT_CONFIG)
        panel_close()

    with right:
        panel_open("Model Boundary", "Batas penggunaan model agar dashboard tetap defensible secara akademik.")
        st.markdown(f"""
        <div class="insight-card">
            <div class="tag">Selected baseline</div>
            <div class="card-title">{selected_model}</div>
            <div class="card-body"><b>ROC AUC:</b> {fmt_float(roc)} — kemampuan ranking risiko sedang.</div>
            <div class="card-body"><b>Average Precision:</b> {fmt_float(ap)} — lebih baik dari baseline {fmt_pct(baseline)}, tetapi belum cukup untuk keputusan otomatis.</div>
            <div class="card-body card-rec">Use case: urutkan prioritas monitoring, bukan memutuskan order pasti terlambat.</div>
        </div>
        """, unsafe_allow_html=True)
        panel_close()

    panel_open("Risk Reliability Check", "Menguji apakah tiap risk bucket cukup besar untuk dijadikan bahan interpretasi.")
    if not risk.empty and {"risk_level", "orders", "actual_late_rate"}.issubset(risk.columns):
        rr = risk.copy()
        rr["risk_label"] = rr.get("risk_label", rr["risk_level"].apply(clean_label))
        total_orders = rr["orders"].sum()
        rr["order_share"] = rr["orders"] / total_orders if total_orders else 0
        rr["reliability"] = np.where(rr["order_share"] >= 0.01, "Readable", "Small sample")
        left, right = st.columns([1.1, 0.9])
        with left:
            fig = px.bar(
                rr.sort_values("orders"),
                x="orders",
                y="risk_label",
                orientation="h",
                text="orders",
                ...
                <div class="rank-card">
                    <div class="rank-left">
                        ...
                        <div>
                            ...
                            <div class="rank-caption">Orders: {fmt_num(row.get('orders', 0))} • {note}</div>
                        </div>
                    </div>
                    <div class="rank-value">{fmt_pct(row.get('actual_late_rate', 0))}</div>
                </div>
                """, unsafe_allow_html=True)
    panel_close()
